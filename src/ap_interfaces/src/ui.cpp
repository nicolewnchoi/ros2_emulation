...
public:
    PublisherNode()
        : Node("UIPublisher"), count_(0)
    {
        publisher_ = this->create_publisher<std_msgs::msg::String>("control", 10);
        auto timer_callback =
            [this]() -> void {
            auto message = std_msgs::msg::String();
            message.data = "Control! " + std::to_string(this->count_++);
...
int main(int argc, char* argv[])
{
    ...
    auto ui_pubnode = std::make_shared<PublisherNode>();
    auto ui_subnode = std::make_shared<SingleThreadedNode>();  // This contains BOTH subscriber callbacks.
                                                          // They will still run on different threads
                                                          // One Node. Two callbacks. Two Threads
    executor.add_node(ui_pubnode);
    executor.add_node(ui_subnode);
    executor.spin();
    rclcpp::shutdown();
    return 0;
}