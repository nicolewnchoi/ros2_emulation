...
public:
    PublisherNode()
        : Node("UnityPublisher"), count_(0)
    {
        publisher_ = this->create_publisher<std_msgs::msg::String>("score", 10);
        auto timer_callback =
            [this]() -> void {
            auto message = std_msgs::msg::String();
            message.data = "Score! " + std::to_string(this->count_++);
...
class TripleThreadedNode : public rclcpp::Node
{
public:
    TripleThreadedNode()
        : Node("UnitySubscribers")
    {
        ...
        subscription1_ = this->create_subscription<std_msgs::msg::String>(
            ...
            std::bind(
                &TripleThreadedNode::subscriber1_cb,  // First parameter is a reference to the function
        ...
        subscription2_ = this->create_subscription<ap_interfaces::msg::Pos>(
            ...
            rclcpp::QoS(10),
            std::bind(
                &TripleThreadedNode::subscriber2_cb,
                this,
                std::placeholders::_1),
            sub2_opt);
        ...
        subscription3_ = this->create_subscription<std_msgs::msg::String>(
            ...
            rclcpp::QoS(10),
            std::bind(
                &TripleThreadedNode::subscriber3_cb,
                this,
                std::placeholders::_1),
            sub3_opt);
    }
...
int main(int argc, char* argv[])
{
    ...
    auto unity_pubnode = std::make_shared<PublisherNode>();
    auto unity_subnode = std::make_shared<TripleThreadedNode>();  // This contains BOTH subscriber callbacks.
                                                          // They will still run on different threads
                                                          // One Node. Two callbacks. Two Threads
    executor.add_node(unity_pubnode);
    executor.add_node(unity_subnode);
    executor.spin();
    rclcpp::shutdown();
    return 0;
}